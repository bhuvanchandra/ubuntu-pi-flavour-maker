...
#FIXME: do we really need the below line
    ...
    chroot $R su ubuntu -c "cd /home/${USERNAME}/catkin_ws/src; git clone https://github.com/rapyuta/io_tutorials.git"
...
#    chroot $R su ubuntu -c "bash -c 'catkin build'"
...
EOM
    ...
    echo "source /etc/rapyuta/env.sh" >> $R/home/ubuntu/.bashrc
    echo "source /etc/rapyuta/env.sh" >> $R/root/.bashrc
    echo "source /etc/rapyuta/env.sh" >> $R/etc/skel/.bashrc
...
function configure_hardware() {
    ...
    chroot $R apt-get -y install libraspberrypi-bin \
    libraspberrypi0 raspberrypi-bootloader rpi-update \
